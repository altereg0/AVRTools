  ...
  int getCurrentByte() {
    ATOMIC_BLOCK(ATOMIC_RESTORESTATE) {
      return mCurrentByte < mTxBufferSize[mCurrentMsg]
             ? mTxBuffer[mCurrentMsg][mCurrentByte++]
             : -1;
    }
  }
...
uint8_t BufferI2cTx::push(uint8_t address, I2cTxMode txMode, const uint8_t *txData, uint8_t txLen,
  ...
  ATOMIC_BLOCK(ATOMIC_RESTORESTATE) {
    ...
    if ((txMode == kI2cRead || txMode == kI2cWriteRestartRead) && (!rxBuf || !rxLen || !rxCnt)) {
...
{
    debugSout->print("Nbr msgs:  ");
    debugSout->println(mNbrMsgs);
    if (mNbrMsgs)
    {
        for (uint8_t i = 0; i < mNbrMsgs; ++i)
        {
            debugSout->print("Msg ");
            debugSout->println(i);
            debugSout->print("Address: ");
            debugSout->println(mAddress[(mCurrentMsg + i) % kMaxNbrMsgs]);
            debugSout->print("Tx mode: ");
            debugSout->println(mTxMode[(mCurrentMsg + i) % kMaxNbrMsgs]);
            uint8_t n = mTxBufferSize[(mCurrentMsg + i) % kMaxNbrMsgs];
            debugSout->print("Tx Size:    ");
            debugSout->println(n);
            ...
            debugSout->print("Rx Size:    ");
            debugSout->println(m);
            debugSout->print("Rx Ptr:     ");
            debugSout->println(*mRxCounter[(mCurrentMsg + i) % kMaxNbrMsgs]);
        }
    }
}
...
uint8_t I2cMaster::writeAsync(uint8_t address, uint8_t registerAddress, const char *data, volatile uint8_t *status) {
  ...
  */
  ...
  uint8_t temp[I2C_MASTER_MAX_TX_MSG_LEN + 1];
  temp[0] = registerAddress;
  memcpy(temp + 1, data, nbrBytes);
