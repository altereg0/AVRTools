...
volatile millis_t timer0_overflow_count;
volatile millis_t timer0_millis;
...
#warning  "Probably not working"
  ...
  // For the 1 MHz internal clock on the any. Example ATMega2560 16MHz with CKDIV8 on

  // 1 cycles = 1 us

  // The overhead of the function call is 14 (16) cycles, which is 16us
  ...
  // The timing loop takes 2 of a microsecond (4 cycles) per iteration
  // So execute it twice for each microsecond of delay requested.
